...
" some shl & other additions
syn keyword PreProc Linux Windows Mac
syn keyword PreProc Clang GCC MSVC
syn keyword numberType  u8 u16 u32 u64 s8 s16 s32 s64
syn keyword mathType    vec2 vec3 vec4 ivec2 ivec3 ivec4 quat mat3x3 mat4x4

syn region  cAssert           start=+\(static_\)\?assert(+ end=+)\s*;+
syn keyword cMemory           init free allocate_memory free_memory reallocate_memory allocate_zeroed_memory
...
if !exists("c_no_if_platform")
    " figure out this lunacy at some point
    " if has('win32')
    "     syn region	cPlatCppOutWrapper	start="^\s*\zs\%(%:\|#\)\s*if\s\+\%(Linux|Mac\)\s*\%($\|//\|/\*\|&\)" end=".\@=\|$" contains=cPlatCppOutIf,cCppOutElse,@NoSpell fold
    "     syn region	cPlatCppOutIf	contained start="\%(Linux|Mac\)" matchgroup=cPlatCppOutWrapper end="^\s*\%(%:\|#\)\s*endif\>" contains=cPlatCppOutIf2,cCppOutElse
    "     syn region	cPlatCppOutIf2	contained matchgroup=cPlatCppOutWrapper start="\%(Linux|Mac\)" end="^\s*\%(%:\|#\)\s*\%(else\>\|elif\s\+\%(\%(Linux\|Mac\)\s*\%($\|//\|/\*\|&\)\)\@!\|endif\>\)"me=s-1 contains=cSpaceError,cPlatCppOutSkip,@Spell fold
    " elseif has('unix')
    "     syn region	cPlatCppOutWrapper	start="^\s*\zs\%(%:\|#\)\s*if\s\+\%(Windows|Mac\)\s*\%($\|//\|/\*\|&\)" end=".\@=\|$" contains=cPlatCppOutIf,cCppOutElse,@NoSpell fold
    "     syn region	cPlatCppOutIf	contained start="\%(Windows|Mac\)" matchgroup=cPlatCppOutWrapper end="^\s*\%(%:\|#\)\s*endif\>" contains=cPlatCppOutIf2,cCppOutElse
    "     syn region	cPlatCppOutIf2	contained matchgroup=cPlatCppOutWrapper start="\%(Windows|Mac\)" end="^\s*\%(%:\|#\)\s*\%(else\>\|elif\s\+\%(\%(Windows\|Mac\)\s*\%($\|//\|/\*\|&\)\)\@!\|endif\>\)"me=s-1 contains=cSpaceError,cPlatCppOutSkip,@Spell fold
    " endif
endif
...
hi def link cPlatCppOutSkip	cPlatCppOut
hi def link cPlatCppInElse2	cPlatCppOut
hi def link cPlatCppOutIf2	cPlatCppOut
hi def link cPlatCppOut		NoFocus
hi def link cAssert		NoFocus

hi def link numberType                  Type
hi def link mathType                    Type
